...
public class InventoryAssistant
{
    ...
    public NewMenuEntry getNewMenuEntry(int itemId, String option)
    {
        WidgetItem itemWidget = getWidgetItem(itemId);
        if (itemWidget != null)
        {
            int id = itemOptionToId(itemWidget.getId(), option);
            return new NewMenuEntry(selectedItemOption(itemWidget.getId(), option), "", id, idToMenuAction(id), itemWidget.getIndex(), WidgetInfo.INVENTORY.getId(), false);
        }
        return null;
    }
    ...
    {
        return getWidgetItems().stream().filter(wi -> wi.getId() == id).findFirst().orElse(null);
    }
    ...
    public int itemOptionToId(int itemId, String option)
    {
        ...
        {
            if (action != null && action.equalsIgnoreCase(option))
            {
                if (index <= 2)
                {
                    return index + 2;
    ...
    public String selectedItemOption(int itemId, String option)
    {
        ItemComposition itemDefinition = getItemDefinition(itemId);
        for (String action : itemDefinition.getInventoryActions())
        {
